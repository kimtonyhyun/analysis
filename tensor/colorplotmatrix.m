function [h,ax,BigAx,hhist,pax] = colorplotmatrix(varargin)
...
for i=rows:-1:1,
    for j=cols:-1:1,
        ...
        xx = reshape(x(:,j,:),[m k]);
        yy = reshape(y(:,i,:),[m k]);
        hh(i,j,:) = plot(xx,yy,sym,'parent',ax(i,j),'color','k')';
        ...
        rr = corr(xx(:),yy(:));
        if rr > 0
            set(gca,'Color',[1-rr^2 1-rr^2 1]);
        else
            set(gca,'Color',[1 1-rr^2 1-rr^2]);
        end
    end
end
