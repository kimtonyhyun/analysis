function make_summary(sources,ic_dir,bg_image)
...
for idx_day = 1:num_days,
    ...
    relevant_maze = eval(['sources.maze',num2str(idx_day)]);
    relevant_behavior_vid_str = eval(['sources.behavior',num2str(idx_day)]);
    ...
    trial_frame_indices = get_trial_frame_indices(relevant_maze);
    compressed_indices = [compressed_indices; last_frame+ ...
        compress_frame_indices(trial_frame_indices,sources.trim)];       
    ...
    frames = [];
    trim_before = sources.trim(1);trim_after = sources.trim(2);
    for k = 1:num_trials_total
        frames = [frames, (trial_frame_indices(k,1)+trim_before):...
            (trial_frame_indices(k,4)-trim_after)];
    end
...
for trial = 1:num_trials_total
    ...
    summary_trial{trial,4} = ica_traces(frames(1):frames(2),:)';
