function norm_movie_by_gaussian(movie_in, movie_out, varargin)
...
if isempty(movie_out)
    ...
    movie_out = sprintf('%s_norm-gauss.hdf5', name);
...
gauss_radius = 35;
if ~isempty(varargin)
    gauss_radius = varargin{1};
end
hDisk = fspecial('gaussian', 3*gauss_radius*[1 1], gauss_radius);
...
title(sprintf('%s (disk radius: %d)', strrep(movie_in,'_','\_'), gauss_radius));
...
h5create(movie_out, '/Norm/GaussRadius', 1, 'Datatype', 'double');
h5write(movie_out, '/Norm/GaussRadius', gauss_radius);
