function norm_movie_by_mean(movie_in, movie_out, varargin)
% Normalize HDF5 movie from file ('movie_in') to file ('movie_out').
% Each frame is divided by its mean.
%
...
%
% Example usage:
%   norm_movie('c9m7d12.hdf5','');
%

if isempty(movie_out)
    [~, name] = fileparts(movie_in);
    movie_out = sprintf('%s_norm-mu.hdf5', name);
...
for i = 1:num_chunks
    ...
    for frame_idx = 1:size(movie_chunk,3);
        ...
        movie_chunk_norm(:,:,frame_idx) = frame ./ mean(frame(:));
    end
